...
async function initRedis() {
    try {
        ...
        console.log('✓ Redis connected successfully on redis://127.0.0.1:6379');
    } catch (err) {
...
app.get(
  ...
  async (req, res) => {
    ...
    const userData = { id: user.id, email: user.email, first_name: user.first_name, last_name: user.last_name, role: user.role };
...
initRedis().then(() => {
  app.listen(PORT, () => {
    ...
    console.log('\nCache configuration:');
    console.log(`  - Users cache: ${USERS_CACHE_TTL} seconds`);
    console.log(`  - Products cache: ${PRODUCTS_CACHE_TTL} seconds\n`);
  });
}).catch((err) => {
